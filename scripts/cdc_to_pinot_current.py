...
class CDCToPinotPipeline:
    def __init__(self):
        ...
        self.pinot_topic_current = 'pinot_sales_current'  # Current state (after data)
    ...
    def create_environment(self):
        ...
        config.set("pipeline.name", "CDC-to-Pinot-Streaming")
    ...
    def run(self):
        ...
        try:
            ...
            # Create both sinks
            self.create_pinot_current_state_sink()
            #self.create_pinot_history_sink()
            
            # Start both pipelines
            logger.info("\nStarting dual pipeline:")
            logger.info(f"  1. Current State → {self.pinot_topic_current}")
            logger.info(f"  2. CDC History → {self.pinot_topic_history}")
            logger.info("")
            ...
            logger.info(f"Sink (Current): {self.pinot_topic_current}")
            #logger.info(f"Sink 2 (History): {self.pinot_topic_history}")
